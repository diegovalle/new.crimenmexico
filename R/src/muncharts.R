...
#                             population, (sum(count) / population)  * 100000 * 12 as rate
...
#                      WHERE (
#                            (tipo_text ='SIN VIOLENCIA' and subtipo_text = 'DE VEHICULOS') and
#                            (date >= date('now', 'start of month', '-13 months'))
#                            )
#                      GROUP BY state_code, mun_code
#                    )
#                    WHERE (population > 50000 AND count >=10)
#                    ORDER BY rate DESC
#                    LIMIT 50")
# muns <- dbGetQuery(db, "
#                    SELECT * FROM (
...
#                      WHERE (
#                            (tipo_text ='SIN VIOLENCIA' and subtipo_text = 'DE VEHICULOS') and
#                            (date == substr(date('now', 'start of month', '-1 months'), 1, 7))
#                            )
#                      GROUP BY state_code, mun_code
#                    )
#                    WHERE (population > 50000 AND count >=10) or (population <= 50000 AND count >= 15)
#                    ORDER BY rate DESC
#                    LIMIT 50")
# 
...
#                    (state_code = 14 and mun_code = 67) or
#                    (state_code = 12 and mun_code = 38) or
# (state_code = 2 and mun_code = 4) or
